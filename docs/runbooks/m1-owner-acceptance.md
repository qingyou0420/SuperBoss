...
blank until a human performs the step against a live deployment. Never record a cookie, token, or
password.

## Release-gate live e2e specs

Before a release, run the Playwright specs in `tests/e2e/specs/` against a live
...
If `E2E_BASE_URL` or any local credential is missing, `npm test` fails before running tests. A failed
or not-run live test remains FAIL or NOT RUN; do not replace it with `npm run test:contracts`,
`npm run test:list`, mocks, or skipped tests.
...
creation, OWNER user listing, and a foreign-project file download; each must return 403. UI hiding
...
annotation, attachment, or HTML report. After the operations, set the OWNER and STAFF actor record IDs plus the exact UTC start/end of the
live acceptance window, then inspect only the required bounded audit rows. The OWNER-only STAFF
denials for project creation and user listing intentionally have a null `object_id`; the
foreign-file denial must carry the recorded file ID.
...
WHERE actor_id IN ('$OwnerActorId'::uuid, '$StaffActorId'::uuid)
  ...
  AND action IN (
    'auth.login', 'project.create', 'file.upload.complete', 'file.download', 'user.list'
  )
...
and the STAFF denials. The expected STAFF denial rows are `project.create` and `user.list` with
`object_id IS NULL`, plus `file.download` with the recorded foreign-file ID. A missing row is FAIL,
