...
class Boto3ObjectStorage:
    ...
    async def presign_upload_part(
        self, key: str, multipart_id: str, part_number: int, expires_seconds: int
    ) -> str:
    ...
    async def complete_multipart(
        self, key: str, multipart_id: str, parts: list[CompletedPart]
    ) -> ObjectMetadata:
        await asyncio.to_thread(lambda: self.client.complete_multipart_upload(Bucket=self.bucket, Key=key, UploadId=multipart_id, MultipartUpload={"Parts": [{"PartNumber": p.part_number, "ETag": p.etag} for p in parts]}))
    ...
    async def abort_multipart(self, key: str, multipart_id: str) -> None:
        await asyncio.to_thread(
            self.client.abort_multipart_upload, Bucket=self.bucket, Key=key, UploadId=multipart_id
        )
    ...
    async def stream(self, key: str) -> AsyncIterator[bytes]:
        body = (await asyncio.to_thread(self.client.get_object, Bucket=self.bucket, Key=key))[
            "Body"
        ]
