"""Repair published knowledge snapshots and align defaults/indexes.

Revision ID: 0018_round3_integrity
Revises: 0017_round2_constraints
"""
...
revision = "0018_round3_integrity"
down_revision = "0017_round2_constraints"
...
def ensure_points_review_audit_schema() -> None:
    op.execute(
        """
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        WHERE r.id = d.published_revision_id
        """
    )
    op.execute(
    ...
    ensure_points_review_audit_schema()
    op.execute(
        f"""
        UPDATE knowledge_revisions r
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS knowledge_revision_review_events")
    op.execute("ALTER TABLE knowledge_revision_pollution DROP COLUMN IF EXISTS reviewed_by")
    op.execute("ALTER TABLE knowledge_revisions DROP COLUMN IF EXISTS points_reviewed_at")
    op.execute("ALTER TABLE knowledge_revisions DROP COLUMN IF EXISTS points_reviewed_by")
    op.execute(
        "ALTER TABLE directory_conflicts DROP CONSTRAINT IF EXISTS ck_directory_conflicts_status"
    )
    op.execute("ALTER TABLE directory_conflicts DROP COLUMN IF EXISTS resolved_at")
    op.execute("ALTER TABLE directory_conflicts DROP COLUMN IF EXISTS resolved_by")
    op.execute("ALTER TABLE directory_conflicts DROP COLUMN IF EXISTS resolved_entry_id")
    op.execute("ALTER TABLE directory_conflicts DROP COLUMN IF EXISTS status")
    op.execute("DROP TABLE IF EXISTS knowledge_revision_pollution")
    op.execute(
        "ALTER TABLE knowledge_revisions DROP CONSTRAINT IF EXISTS "
        "ck_knowledge_revisions_points_review"
    )
    op.execute("ALTER TABLE knowledge_revisions DROP COLUMN IF EXISTS points_review")
    op.execute("ALTER TABLE knowledge_revisions DROP COLUMN IF EXISTS released")
